...
def load_model(
    ...
    ).to("cuda", dtype=torch.bfloat16)  # Move model to GPU
...
def sequence_log_probs_from_logits(
    ...
    log_prob = F.log_softmax(logits.to(torch.float16), dim=-1)
    ...
    for k, prompt_batch in enumerate(prompt_loader):
        ...
        for step_epoch in range(epochs_per_step):
            ...
            for i, exp in enumerate(experience_sampler):
                exp = exp.to("cuda")  # Move experience batch to GPU
                ...
                if not loss.isfinite():
                    ...
                    print(f"experience.advantages={exp.advantages}")  # Use `exp` instead of `experience`
                    continue
